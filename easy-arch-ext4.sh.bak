...
# Switch to installed non-root user since yay does not like installing with root.
su $username
cd ~/
...
pacman -S --noconfirm - < yaylist.txt

# Switch back to root
exit

git clone https://git.wretchednet.com/wretchedghost/easy-arch-i3
cd easy-arch-i3
systemctl enable lightdm
cp background.jpg /usr/share/pixmaps/
rm -rf /etc/lightdm/
cp -R lightdm /etc/lightdm

chown -R $username:$username ../easy-arch-i3-gui

su $username
rsync -av .bashrc .config .fehbg .tmux.conf .vim .vimrc .xinitrc .Xresources Pictures ~/

info_print "Gui and i3 Linux have been setup now."
